...
# Import df :
df_rein = pd.read_csv('/data/df_ckd_7.csv')
df_foie = pd.read_csv('/data/dataset_foie_ML.csv')
df_diabete = pd.read_csv('/data/df_diabete.csv')
df_sein = pd.read_csv('/data/dataset_sein_requilibre.csv')
df_coeur = pd.read_csv('/data/df_coeur.csv')
...
dbc.Row(html.P(style={'margin-top': '20px'})),
    ...
    dbc.Row(dbc.Col([dbc.InputGroup([
                ...
                dbc.Input(id='Input_IDdiab', type='text', 
                        placeholder= "Entrer l'ID du patient")])
                        ], className="d-grid gap-2 col-6 mx-auto")),        
    dbc.Row(html.P(style={'margin-top': '20px'})),
    dbc.Row(html.H3("Entrer vos informations ci dessous :")),
    dbc.Row(html.P(style={'margin-top': '20px'})),
    dbc.Row(dbc.Col([
            dbc.InputGroup([
                    dbc.InputGroupText('Age'),
                    dbc.Input(id='Input_agediab', type='number', placeholder= 'Entrer un nombre')                  
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText('Nombre de grossesses'),
                    dbc.Input(id='Input_grossesse', type='number', placeholder= 'Entrer un nombre')                  
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText('Indice de masse corporelle'),
                    dbc.Input(id='Input_imc', type='number', placeholder= 'Entrer un nombre')                  
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText('Insuline sérique'),
                    dbc.Input(id='Input_insuline', type='number', placeholder= 'Entrer un nombre')                  
                ]),  
            dbc.InputGroup([
                    dbc.InputGroupText("Glucose"),
                    dbc.Input(id='Input_glucose', type='number', placeholder= 'Entrer un nombre')                  
                ]),  
            dbc.InputGroup([
                    dbc.InputGroupText('Epaisseur pli cutané du triceps'),
                    dbc.Input(id='Input_triceps', type='number', placeholder= 'Entrer un nombre')                  
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText('Pression artérielle'),
                    dbc.Input(id='Input_pressart', type='number', placeholder= 'Entrer un nombre')                  
                ]), 
            dbc.InputGroup([
                    dbc.InputGroupText("Diabete Pedigree Fonction"),
                    dbc.Input(id='Input_proba', type='number', placeholder= 'Entrer un nombre')                  
                ])
        ], className="d-grid gap-2 col-6 mx-auto")),
    dbc.Row(html.P(style={'margin-top': '20px'})),
...
dbc.Row(html.P(style={'margin-top': '20px'})),
    ...
    dbc.Row(dbc.Col([
            dbc.InputGroup([
                    dbc.InputGroupText('Age'),
                    dbc.Input(id='Input_age', type='number', placeholder= 'Entrer un nombre')                  
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText('Volume de globules rouges'),
                    dbc.Input(id='Input_pcv', type='number', placeholder= 'Entrer un nombre')                  
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText("Pression artérielle"),
                    dbc.Input(id='Input_pressure', type='number', placeholder= 'Entrer un nombre')                   
                ]),
    ], className="d-grid gap-2 col-6 mx-auto")),
    dbc.Row(html.P(style={'margin-top': '20px'})),
    dbc.Row(dbc.Col([
...
dbc.Row(html.P(style={'margin-top': '20px'})),
    ...
    dbc.Row([
        dbc.Col([dbc.InputGroup([
                    dbc.InputGroupText('Age '),
                    dbc.Input(id='Input_agefoie', type='number', placeholder= 'Entrer un nombre')                  
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText('Alamine Aminotransferase'),
                    dbc.Input(id='Input_alamine', type='number', placeholder= 'Entrer un nombre')                  
                ]),
            dbc.InputGroup([
            ...
            dbc.InputGroup([
                    ...
                    dbc.Input(id='Input_bili', type='number', placeholder= 'Entrer un nombre')                   
                ])], className="d-grid gap-2 col-6 mx-auto")
    ]),
    dbc.Row(html.P(style={'margin-top': '20px'})),
...
dbc.Row(html.P(style={'margin-top': '20px'})),
    ...
    dbc.Row([
        dbc.Col([dbc.InputGroup([
                    dbc.InputGroupText("Age"),
                    dbc.Input(id='Input_agecoeur', type='number', placeholder= 'Entrer un nombre')                   
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText('Pression artérielle'),
                    dbc.Input(id='Input_pad', type='number', placeholder= 'Entrer un nombre')                  
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText('Cholesterol'),
                    dbc.Input(id='Input_chol', type='number', placeholder= 'Entrer un nombre')                   
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText("Glycémie"),
                    dbc.Input(id='Input_glycemie', type='number', placeholder= 'Entrer un nombre')                   
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText("Fréquence cardiaque max"),
                    dbc.Input(id='Input_freqmax', type='number', placeholder= 'Entrer un nombre')                   
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText("Type de Thalassémie"),
                    dbc.Input(id='Input_thalass', type='number', placeholder= 'Entrer un nombre')                   
                ]), 
            dbc.InputGroup([
                    dbc.InputGroupText("Dépression Segment ST"),
                    dbc.Input(id='Input_depression', type='number', placeholder= 'Entrer un nombre')                   
                ]),
            dbc.InputGroup([
                    dbc.InputGroupText("Pente Segment ST"),
                    dbc.Input(id='Input_pente', type='number', placeholder= 'Entrer un nombre')                   
                ]),              
            dbc.InputGroup([
                    dbc.InputGroupText("Nb vaisseaux majeurs colorés par fluoroscopie"),
                    dbc.Input(id='Input_vaisseaux', type='number', placeholder= 'Entrer un nombre')                   
                ])],className="d-grid gap-2 col-6 mx-auto")         
    ]),
    dbc.Row(html.P(" ")),
...
sein_layout = html.Div([
    ...
    dbc.Row(html.P(style={'margin-top': '20px'})),
    dbc.Row(html.H3("Entrer vos informations sur la cellule ci-dessous :")),
    dbc.Row(html.P(style={'margin-top': '20px'})),
    dbc.Row([
        dbc.Col([
            dbc.InputGroup([
                dbc.InputGroupText('Texture Mean :'),
                dbc.Input(id='Input_texture_mean', type='number', placeholder='Entrer un nombre')
            ]),
            dbc.InputGroup([
                dbc.InputGroupText('Area mean :'),
                dbc.Input(id='Input_area_mean', type='number', placeholder='Entrer un nombre')
            ]),
            dbc.InputGroup([
                dbc.InputGroupText('Smoothness mean :'),
                dbc.Input(id='Input_smoothness_mean', type='number', placeholder='Entrer un nombre')
            ]),
            dbc.InputGroup([
                dbc.InputGroupText('Compactness mean :'),
                dbc.Input(id='Input_compactness_mean', type='number', placeholder='Entrer un nombre')
            ]),
            dbc.InputGroup([
                dbc.InputGroupText('Concavity mean :'),
                dbc.Input(id='Input_concavity_mean', type='number', placeholder='Entrer un nombre')
            ]),
            dbc.InputGroup([
                dbc.InputGroupText('Concave points mean :'),
                dbc.Input(id='Input_concave_points_mean', type='number', placeholder='Entrer un nombre')
            ]),
            dbc.InputGroup([
                dbc.InputGroupText('Symmetry mean :'),
                dbc.Input(id='Input_symmetry_mean', type='number', placeholder='Entrer un nombre')
            ]),
        ], className="d-grid gap-2 col-6 mx-auto"),
    ]),
    dbc.Row(html.P(style={'margin-top': '20px'})),
